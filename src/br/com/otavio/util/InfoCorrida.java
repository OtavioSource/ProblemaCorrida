...
public Map<String, List<Volta>> retornaMapDeVoltas(){
	
	
	CarregaInformacaoArquivo util = new CarregaInformacaoArquivo();
	List<Volta> voltas = new ArrayList<>();
	try {
		voltas = util.infoVoltas();
	} catch (IOException e) {
...
public List<Piloto> retornaListaDePiloto(){
	
	
	List<Piloto> listaPilotosComInfoDeCadaVolta = new ArrayList<>();
	
	Map<String, List<Volta>> agrupamentoPorCodigoDoPiloto = retornaMapDeVoltas();
	...
	agrupamentoPorCodigoDoPiloto.forEach((k,v) ->{
	    ...
	    piloto.setTempoTotalProva(v.get(v.size()-1).getHoraVolta().minus(v.get(0).getHoraVolta().toNanoOfDay(), ChronoUnit.NANOS));	    
...
public List<Piloto> ordenaListaPilotos(List<Piloto> listaPilotos){

	
	Comparator<Piloto> comparatorNumeroVolta = (p1,p2) -> p1.getNumeroUltimaVolta().compareTo(p2.getNumeroUltimaVolta());
	Comparator<Piloto> comparatorTempoProva = (p1,p2) -> p1.getTempoTotalProva().compareTo(p2.getTempoTotalProva());
